...
<#
    ...
    PSToolboxExample.Tests.ps1:
      ...
      - Mocking a private (non-exported) function via -ModuleName, to verify
        Install-WinGetPackage calls out correctly without ever touching the
        real WinGet client.
...
Describe 'Export-WinGetPackage' {
    ...
    It 'exports only the non-MSIX, non-ARP packages' {
        ...
        Export-WinGetPackage -Path $path
    ...
    It 'uses the specified delimiter' {
        ...
        Export-WinGetPackage -Path $path -Delimiter ','
    ...
    It 'does not create a file when no packages are found' {
        ...
        Export-WinGetPackage -Path $path -WarningAction SilentlyContinue
    ...
    It 'does not overwrite a file that already exists' {
        ...
        Export-WinGetPackage -Path $path -ErrorAction SilentlyContinue

        Get-Content -Path $path -Raw | Should -Match 'do-not-touch-me'
    }
}

Describe 'Import-WinGetPackage' {

    BeforeAll {
        Mock -ModuleName Ictstuff.Windows.PackageManagement Install-WinGetPackage { }
    }

    It 'calls Install-WinGetPackage with the package IDs from the CSV' {
        ...
        Import-WinGetPackage -Path $path
    ...
    It 'does not call Install-WinGetPackage when the file does not exist' {
        Import-WinGetPackage -Path (Join-Path $TestDrive 'missing.csv') -ErrorAction SilentlyContinue

        Should -Invoke -ModuleName Ictstuff.Windows.PackageManagement Install-WinGetPackage -Times 0
    }

    It 'does not call Install-WinGetPackage when the CSV has no Id column' {
        $path = Join-Path $TestDrive 'no-id-column.csv'
        @([pscustomobject]@{ Name = 'Git' }) | Export-Csv -Path $path -Delimiter ';' -NoTypeInformation

        Import-WinGetPackage -Path $path -ErrorAction SilentlyContinue
    ...
    It 'does not install anything when -WhatIf is specified' {
        ...
        Import-WinGetPackage -Path $path -WhatIf
...
Describe 'Install-WinGetPackage' {
    ...
    BeforeAll {
        Mock -ModuleName Ictstuff.Windows.PackageManagement Invoke-WinGetPackageInstall { }
    }
    ...
    It 'skips a package that is already installed' {
        ...
        Should -Invoke -ModuleName Ictstuff.Windows.PackageManagement Invoke-WinGetPackageInstall -Times 0
    }
    ...
    It 'installs a package that is not already installed' {
        ...
        Should -Invoke -ModuleName Ictstuff.Windows.PackageManagement Invoke-WinGetPackageInstall -Times 1 -Exactly -ParameterFilter {
    ...
    It 'accepts multiple package IDs from the pipeline' {
        ...
        'Git.Git', 'Microsoft.VisualStudioCode' | Install-WinGetPackage

        Should -Invoke -ModuleName Ictstuff.Windows.PackageManagement Invoke-WinGetPackageInstall -Times 2 -Exactly
    }

    It 'does not install anything when -WhatIf is specified' {
        Mock -ModuleName Ictstuff.Windows.PackageManagement Get-WinGetPackage { $null }

        Install-WinGetPackage -PackageId 'Git.Git' -WhatIf
